...
public class TestGraphs {
	public static void main( String args[] ) throws ParseException {
		...
		node2.setIn(node2in);*/
		String someJson = "{'timestamp':'1999-10-23','name':'A nice name','id':'a_urlsafe_name','ends': ['nodeadd1'],'nodes': [{'name':'nodeadd1','url':'http://localhost:8080/blah/blah/add','params':[{'name':'num1','value':5},{'name':'num2','from':'nodeadd2'}]},{'name':'nodeadd2','url':'http://localhost:8080/blah/blah/add','params': [{'name':'num1','value':3},{'name':'num2','value':5}]}]}";
		JsonParser parser = new JsonParser();
		JsonObject o = parser.parse(someJson).getAsJsonObject();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-mm-dd");
		System.out.println(sdf.parse(o.get("timestamp").toString().replace("\"", "")));
	}
}
